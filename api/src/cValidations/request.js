const validatePost = async(body, next, Service) => {
    ...
    if(service === null) return next({status: 400, message:"El Servicio solicitado no existe"});

    if(typeof body.contact !== "object") return next({status: 400, message:"Los valores del campo contacto son invalidos, modifiquelos"});
    if(body.contact.length < 1) return next({status: 400, message:"no se puede postear sin contacto"});
    if(body.thg !== body.contact[0].type) return next({status: 400, message:"el medio no coincide con el contacto"});
};
